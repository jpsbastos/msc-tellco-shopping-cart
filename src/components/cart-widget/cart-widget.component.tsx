...
export class CartWidgetComponent extends React.Component<{}> {
    ...
    public componentDidMount(): void {
        addEventListener('signin', (e: CustomEvent) => {
           apiService.checkShoppingCart(e.detail.userId).then((resp: { data: {[key: string]: any}}) => {
        ...
        addEventListener('update-cart', (e: CustomEvent) => {
            apiService.addProductToCart(e.detail.product._id).then((resp: { data: {[key: string]: any}}) => {
                this.setState({ total : resp.data.totalItems })
            });
        })
    }
    ...
    public render(): JSX.Element {
        return(
            <div className="cart-widget" onClick={() => this.widgetClicked()}><FontAwesomeIcon icon={['fas', 'shopping-cart']} /><span>{this.state.total}</span>
            </div>
        );
    }
